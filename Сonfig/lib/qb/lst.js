...
//            var x1 = console.log.bind(console);
//            x1("!!!!!!"+column);
// BRITIN REF=>FK
        var joins = column.fk
  //   BRITIN FK replace ###
  //          ? [z.join(table, column.fk.replace(/:###:/g,'\"'), ref, ref.pk, alias)]
...
function statement (table, columns, filter, joins) {
    ...
    for (var i=0; i < columns.length; i++) {
        ...
        if (!value) continue;
// BRITIN column => oneToMany.column
        if (column.oneToMany && column.oneToMany.fk) {
            if (value == 'NULL') continue;
            statements.push(x.and(z.eq(table, column.oneToMany.fk, value)));
        }
        else if (column.manyToMany) {
