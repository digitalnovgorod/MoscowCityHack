...
var moment = require('moment'),
    async = require('async');
// BRITIN 09.2020 EX Client = require('./lib/db/client'),
...
function initServer (args) {
    ...
    (function () {
        ...
        if (have && _routes.custom) {
          x('DO KEY');
          x(key);
          x(_routes.custom);
          /*  switch(key){
              case 'widgetbar':
                  app.all(_routes.custom, r.auth.restrict, r.editview.get);
                  break;
              default:
                  app.all(_routes.custom, r.auth.restrict, r.render.admin);
            }*/
            app.all(_routes.custom, r.auth.restrict, r.render.bar);
